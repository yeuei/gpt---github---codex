...
class TriggerTests(unittest.TestCase):
    ...
    def test_dashboard_contains_repository_configuration_and_pr_timeline_surfaces(self):
        ...
        self.assertIn('self.path in {"/", "/pair"}', source)
        self.assertIn("location.origin+'/pair#invite='", trigger.HTML)
        self.assertIn('URL fragment', trigger.HTML)
        self.assertIn('"/pair"', source)
        self.assertIn("location.origin+'/pair#invite='", trigger.HTML)
        self.assertIn('URL fragment', trigger.HTML)
        self.assertIn('一次性配对链接', trigger.HTML)
        self.assertIn('token', trigger.HTML)
